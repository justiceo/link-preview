...
function ExtPay(extension_id) {
  ...
  management &&
    management.getSelf().then(async (ext_info) => {
...
`;
  ...
  async function fetch_user() {
    ...
    if (parsed_user.trialStartedAt) {
      if (
        !storage.extensionpay_user ||
        (storage.extensionpay_user && !storage.extensionpay_user.trialStartedAt)
      ) {
